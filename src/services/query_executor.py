"""
Interactive query executor for running CPGQL queries with persistent Joern shell in Docker containers
"""
import asyncio
import json
import logging
...
from ..models import QueryResult, QueryConfig, JoernConfig
from ..exceptions import QueryExecutionError
from ..utils.redis_client import RedisClient
from ..utils.validators import validate_cpgql_query, hash_query
...
class QueryExecutor:
    ...
    async def get_query_status(self, query_id: str) -> Dict[str, Any]:
        ...
        if "completed_at" in status_info and "started_at" in status_info:
            status_info["execution_time"] = status_info["completed_at"] - status_info["started_at"]
    ...
    async def get_query_result(self, query_id: str) -> QueryResult:
        ...
        if status_info["status"] != QueryStatus.COMPLETED.value:
            raise QueryExecutionError(f"Query {query_id} is not completed yet (status: {status_info['status']})")
    ...
    async def _get_container_id(self, session_id: str) -> Optional[str]:
        ...
        if self.cpg_generator:
            ...
            logger.debug(f"Got container ID from CPG generator for session {session_id}: {container_id}")
            return container_id
        container_id = self.session_containers.get(session_id)
        logger.debug(f"Got container ID from local cache for session {session_id}: {container_id}")
        return container_id
    ...
    async def _read_file_from_container(self, session_id: str, file_path: str) -> str:
        ...
        try:
            ...
            if result.exit_code == 0:
                ...
                raise QueryExecutionError(f"Failed to read file {file_path}: exit code {result.exit_code}")
    ...
    async def execute_query(
        ...
        try:
            ...
            result = await self._execute_query_in_shell(session_id, query_normalized, timeout_val)
        ...
        if query_id in self.query_status:
            ...
            if "output_file" in status_info:
                try:
                    ...
                    logger.warning(f"Failed to cleanup output file for query {query_id}: {e}")
    ...
    async def cleanup_old_queries(self, max_age_seconds: int = 3600):
        ...
        for query_id, status_info in self.query_status.items():
            if status_info["status"] in [QueryStatus.COMPLETED.value, QueryStatus.FAILED.value]:
                age = current_time - status_info.get("completed_at", status_info["created_at"])
    ...
    async def _load_cpg_in_container(self, session_id: str, cpg_path: str):
        ...
        logger.info(f"Loading CPG {cpg_path} in container {container_id} for session {session_id}")
...
'''
            ...
            if script_result.exit_code != 0:
                error_output = script_result.output.decode('utf-8', errors='ignore') if script_result.output else "No output"
                logger.error(f"Failed to create CPG loading script: {error_output}")
                raise QueryExecutionError(f"Failed to create CPG loading script: {error_output}")
            
            # Execute the script
            load_result = container.exec_run(["/bin/bash", "/tmp/load_cpg.sh"])
            
            if load_result.exit_code != 0:
                error_msg = load_result.output.decode('utf-8', errors='ignore') if load_result.output else "No output"
    ...
    async def _execute_query_in_shell(self, session_id: str, query: str, timeout: int) -> QueryResult:
...
'''
            ...
            if script_result.exit_code != 0:
                ...
                raise QueryExecutionError(f"Failed to create query script: {error_output}")
            
            # Execute the script
            loop = asyncio.get_event_loop()
            
            def _exec_sync():
                result = container.exec_run(["/bin/bash", "/tmp/query.sh"], workdir="/workspace")
                return result
            ...
            if exec_result.exit_code != 0:
                ...
                logger.error(f"Query execution failed with exit code {exec_result.exit_code}: {output}")
                return QueryResult(
                    success=False,
                    error=f"Query execution failed: {output}"
                )
