...
def register_tools(mcp, services: dict):
    ...
    @mcp.tool()
    async def list_queries(session_id: Optional[str] = None) -> Dict[str, Any]:
        """
        List default CPGQL queries for code analysis and vulnerability detection.
        
        Returns a comprehensive list of predefined queries for common code analysis tasks,
        including generic browsing, security vulnerability detection, and memory safety checks.
        
        Args:
            session_id: Filter queries by session ID (optional, for backward compatibility)
        ...
        Returns:
            {
                ...
                "queries": [
                    {
                        "name": "All Methods",
                        "description": "List all method/function names in the codebase",
                ...
                "total": 15,
                "categories": ["browsing", "security", "memory", "structure"]
            }
        """
        # Default CPGQL queries for various analysis tasks
        default_queries = [
            {
                "name": "All Methods",
                "description": "List all method/function names in the codebase",
            ...
            },
            {
                ...
                "description": "List all function call names in the codebase",
                "query": "cpg.call.name.l",
                "category": "browsing"
            },
            {
                "name": "All Classes/Types",
                "description": "List all class and type declarations",
            ...
            {
                "name": "All Imports",
                "description": "List all import statements",
                "query": "cpg.import.code.l",
                "category": "structure"
            },
            {
                "name": "All String Literals",
                "description": "List all string literals in the code",
                "query": "cpg.literal.code.l",
                "category": "browsing"
            },
            {
                "name": "SQL Injection Vulnerabilities",
                "description": "Find potential SQL injection vulnerabilities",
                "query": "cpg.call.name(\".*execute.*\").argument.code(\".*\\\\+.*\").l",
                "category": "security"
            },
            {
                "name": "Command Injection",
                "description": "Find potential command injection vulnerabilities",
                "query": "cpg.call.name(\"system|exec|popen\").argument.code(\".*\\\\+.*\").l",
                "category": "security"
            },
            {
                "name": "Hardcoded Secrets",
                "description": "Find hardcoded passwords, secrets, and tokens",
                "query": "cpg.literal.code(\".*(password|secret|key|token|credential).*\").l",
                "category": "security"
            },
            {
                "name": "Path Traversal",
            ...
            },
            {
                ...
                "description": "Find unsafe buffer operations that may cause overflows",
                "query": "cpg.call.name(\"strcpy|strcat|strncpy|strncat|gets|sprintf|vsprintf\").l",
                "category": "memory"
            },
            {
                "name": "Memory Allocation",
                "description": "Find all memory allocation calls",
                "query": "cpg.call.name(\"malloc|calloc|realloc|new\").l",
                "category": "memory"
            },
            {
                "name": "Memory Deallocation",
                "description": "Find all memory deallocation calls",
                "query": "cpg.call.name(\"free|delete\").l",
                "category": "memory"
            },
            {
                "name": "Null Pointer Dereferences",
                "description": "Find potential null pointer dereferences",
                "query": "cpg.call.argument.isNull.l",
                "category": "memory"
            },
            {
                "name": "Uninitialized Variables",
                "description": "Find potentially uninitialized local variables",
                "query": "cpg.local.whereNot(_.definingBlock).l",
                "category": "memory"
            },
            {
                "name": "Control Structures",
                "description": "List all control structures (if, while, for, etc.)",
                "query": "cpg.controlStructure.code.l",
                "category": "structure"
            },
            {
                "name": "Return Statements",
                "description": "List all return statements",
                "query": "cpg.return.code.l",
                "category": "structure"
            },
            {
                "name": "XSS Vulnerabilities",
                "description": "Find potential cross-site scripting vulnerabilities",
                "query": "cpg.call.name(\".*write.*\").argument.code(\".*(request|param).*\").l",
                "category": "security"
            },
            {
                "name": "All Parameters",
                "description": "List all function parameters",
                "query": "cpg.parameter.name.l",
                "category": "structure"
            },
            {
        ...
        categories = list(set(q["category"] for q in default_queries))
        
        # For backward compatibility, if session_id is provided, also include running queries
        if session_id:
            try:
                query_executor = services['query_executor']
                running_queries = await query_executor.list_queries(session_id)
                
                return {
                    "success": True,
                    "default_queries": default_queries,
                    "running_queries": running_queries,
                    "total_default": len(default_queries),
                    "total_running": len(running_queries),
                    "categories": categories
                }
            except Exception as e:
                logger.warning(f"Could not fetch running queries: {e}")
