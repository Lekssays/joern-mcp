...
from src.models import QueryResult, QueryConfig, JoernConfig
...
class TestQueryExecutor:
    ...
    def test_normalize_query_for_json_with_offset_and_limit(self, query_executor):
        ...
        result = query_executor._normalize_query_for_json("cpg.method", limit=10, offset=5)
    ...
    async def test_execute_query_success(self, query_executor):
        ...
        with patch.object(query_executor, '_get_container_id', return_value='container-123'), \
             patch.object(query_executor, '_ensure_cpg_loaded') as mock_ensure, \
    ...
    async def test_execute_query_async_invalid_query(self, query_executor):
        ...
        with patch('src.services.query_executor.validate_cpgql_query', side_effect=ValueError("Invalid query")):
            with pytest.raises(QueryExecutionError, match="Query initialization failed"):
    ...
    async def test_get_query_status_found(self, query_executor):
        ...
        query_executor.query_status[query_id] = {
            ...
            "completed_at": 1001.5,
            "started_at": 1000.5,
            "result": {"success": True, "data": [], "row_count": 0, "execution_time": 1.0}
        }
    ...
    async def test_get_query_result_not_completed(self, query_executor):
        ...
        with pytest.raises(QueryExecutionError, match="Query test-query-id is not completed yet"):
    ...
    async def test_cleanup_query_success(self, query_executor):
        ...
        with patch.object(query_executor, '_get_container_id', return_value='container-123'), \
             patch.object(query_executor.docker_client.containers, 'get', return_value=mock_container):
    ...
    async def test_cleanup_all(self, query_executor):
        ...
        with patch.object(query_executor, 'cleanup_query') as mock_cleanup_query, \
             patch.object(query_executor, 'close_session') as mock_close_session:
    ...
    async def test_execute_query_with_special_characters(self, query_executor):
        ...
        with patch.object(query_executor, '_get_container_id', return_value='container-123'), \
            ...
            result = await query_executor.execute_query(
                session_id="test-session",
                cpg_path="/workspace/cpg.bin",
                query='cpg.method.name("test; DROP TABLE users; --")'  # SQL injection-like
            )
